...
class AuthUserSerializerTest(TestCase):
    ...
    def test_registration_serializer_valid_data(self):
        ...
        request_data = {
            "user": {"email": "letschuka@gmail.com", "password": "12345678"}
        }
        response = client.post(
            "/users/", json.dumps(request_data), content_type="application/json"
        )
    ...
    def test_social_link_serializer(self):
        social_link_data = {"link": "http://example.com", "user": self.user}
    ...
    def test_author_serializer(self):
        user = AuthUser.objects.create_user(
            email="example11@gmail.com", password="12345678"
        )
        ...
        author_data = {
            ...
            "social_links": [
                {"id": 1, "link": "http://social1.com"},
                {"id": 2, "link": "https://social2.com"},
            ],
        }
        ...
        self.assertEqual(
            len(serialized_data["social_links"]), len(author_data["social_links"])
        )
        for i in range(len(author_data["social_links"])):
            self.assertEqual(
                serialized_data["social_links"][i]["link"],
                author_data["social_links"][i]["link"],
            )
